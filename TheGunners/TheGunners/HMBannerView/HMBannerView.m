...
//        @property(nonatomic,retain) UIColor *currentPageIndicatorTintColor
        ...
        self.pageControl.currentPageIndicatorTintColor = kThemeRedColor;
...
- (void)startDownloadImage
{
    ...
    for (NSInteger i=0; i<self.imagesArray.count; ++i)
    {
        ...
        if ([url isNotEmpty])
        {
            ...
            /*
            [[SDWebImageManager sharedManager] downloadWithURL:[NSURL URLWithString:url] options:SDWebImageProgressiveDownload progress:^(NSInteger receivedSize, NSInteger expectedSize) {
            ...
            //change to pre version
            /*
            [[SDWebImageManager sharedManager] downloadWithURL:[NSURL URLWithString:url] options:SDWebImageProgressiveDownload progress:^(NSUInteger receivedSize, long long expectedSize) {
...
- (void)refreshScrollView
{
    ...
    for (NSInteger i = 0; i < 3; i++)
    {
        ...
        if (imageView && [imageView isKindOfClass:[UIImageView class]] && [url isNotEmpty])
        {
            ...
            if ([imageCache diskImageExistsWithKey:url]) {
                [imageView setImage:[imageCache imageFromDiskCacheForKey:url]];
            } else {
                [imageView sd_setImageWithURL:[NSURL URLWithString:url]
                             placeholderImage:[UIImage imageNamed:@"home_banner_none"]];
            }
            
            //[imageView sd_setImageWithURL:[NSURL URLWithString:url] placeholderImage:[UIImage imageNamed:@"home_banner_none"]];
        }
    }
